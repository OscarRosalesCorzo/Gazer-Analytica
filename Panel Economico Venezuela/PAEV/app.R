...
#setwd("C:/Users/Oscar Rosales/Desktop/Gazer Analytica/Panel Economico Venezuela/PAEV")
#getwd()

inflacion <- read_excel("PEV_Data.xlsx", 
...
tipo_cambio <- read_excel("PEV_Data.xlsx", 
                          sheet = "Tipo de Cambio", col_types = c("date", 
                                                                  "numeric", "numeric", "skip"), skip = 2)


remuneracion <- read_excel("PEV_Data.xlsx", 
                           sheet = "Remuneracion Trabajadores", 
                           col_types = c("date", "numeric", "numeric", 
                                         "numeric", "numeric"), skip = 2)


petroleo <- read_excel("PEV_Data.xlsx", 
                   ...
                   tabPanel("Tipo de Cambio",
                            ...
                            sidebarPanel(
                              ...
                              selectInput(inputId = "typetc", label = strong("Ver el tipo de cambio como:"),
                                          choices = c("Precio de la Divisa","Diferencia Porcentual: Oficial vs Paralelo"),
...
server <- function(input, output) {
  ...
  output$chart_divisa <- renderPlot({
    ...
    if ( xtc() == "Precio de la Divisa" ) {
      ...
      #Calcuculos previos
      TC() %>% 
        mutate(porcentual = (`Dólar No Oficial`/`Dólar BCV`) - 1) %>% 
        #ggplot
        ggplot(aes(x = Fecha))+
        #geoms
  ...
  output$chart_inflacion <- renderPlot({
    ...
    if (  xin() == "Mensual" ) {
      ...
      IN() %>% 
        #Data manipulation
        select(Fecha, `Inflacion Mensual OVF`) %>%
        #ggplot
        ggplot()+
        #geoms
        geom_line(aes( x = Fecha  ,  y = `Inflacion Mensual OVF`))+
        #scales
        scale_y_continuous(labels = scales::percent,
                           limits = c(0,NA))+
        #theme
      ...
      if( xin() == "Interanual" ) {
        ...
        IN() %>% 
          #Data manipulation
          select(Fecha , `Inflacion Interanual OVF` ) %>% 
          #ggplot
          ggplot()+
          #geom
          geom_line(aes( x =  Fecha, y = `Inflacion Interanual OVF` ))+
          #scales
          scale_y_continuous(labels = scales::percent,
                             limits = c(0,NA))+
          #theme
        ...
        IN() %>% 
          #Data manipulation
          select(Fecha, `Inflacion Acumulada OVF`) %>% 
          ...
          ggplot()+
          #geom
          geom_line(aes( x= Fecha, y = `Inflacion Acumulada OVF`, col =ano ) )+
          #scales
          scale_y_continuous(labels = scales::percent, 
                             limits = c(0,NA))+
